...
const createProduct = async (req, res) => {
  try {
    const { name, slug, price, description, category, shipping, quantity } =
      req.fields;
...
const deleteProduct = async (req, res) => {
  try {
    const products = await Product.findByIdAndDelete(req.params.pid).select(
      "-photo"
    );
    res.status(200).send({
      success: true,
...
const productByCategory = async (req, res) => {
  try {
    ...
    const products = await Product.find({ category: categorys._id }).populate(
      "category"
    );
    res.status(200).send({
      success: true,
...
const brainTreePaymentController = async (req, res) => {
  try {
    ...
    let total = cart.reduce((acc, item) => acc + item.price, 0); // Use reduce to calculate total
    gateway.transaction.sale(
      {
        amount: total.toFixed(2), // Ensure the amount is formatted to two decimal places
